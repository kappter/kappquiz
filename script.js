...
function generateAnalogousColors() {
    const baseHue = Math.floor(Math.random() * 360); // Random hue (0-359)
    const saturation = 70; // Vibrant but not overwhelming
    const lightness = 50; // Balanced for readability
    const colors = [
        `hsl(${baseHue}, ${saturation}%, ${lightness}%)`, // Base color
        `hsl(${(baseHue + 30) % 360}, ${saturation}%, ${lightness}%)`, // +30 degrees
        `hsl(${(baseHue + 60) % 360}, ${saturation}%, ${lightness - 10}%)`, // +60 degrees, darker
        `hsl(${(baseHue - 30 + 360) % 360}, ${saturation}%, ${lightness + 10}%)` // -30 degrees, lighter
    ];
    return colors;
}
    ...
    if (theme === 'random') {
        ...
        styleSheet.textContent = `
            ...
            body.random button:hover {
                background-color: hsl(${(parseInt(buttonColor.match(/\d+/)[0]) + 30) % 360}, 70%, 40%);
            }
            body.random #report-container th {
                background-color: ${thColor};
                color: ${textColor};
            }
            body.random #report-container th, body.random #report-container td {
